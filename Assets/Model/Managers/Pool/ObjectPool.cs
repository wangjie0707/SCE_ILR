    ...
    public sealed class ObjectPool<T> : ObjectPoolBase, IObjectPool<T> where T : ObjectBase
    {
        ...
        public override int Capacity
        {
            ...
            set
            {
                ...
                Log.Info("Object pool '{0}' capacity changed from '{1}' to '{2}'.", TextUtil.GetFullName<T>(Name), m_Capacity.ToString(), value.ToString());
                m_Capacity = value;
                Release();
            }
        }
        ...
        public override float ExpireTime
        {
            ...
            set
            {
                ...
                Log.Info("Object pool '{0}' expire time changed from '{1}' to '{2}'.", TextUtil.GetFullName<T>(Name), m_ExpireTime.ToString(), value.ToString());
                m_ExpireTime = value;
                Release();
            }
        }
        ...
        public void Register(T obj, bool spawned)
        {
            ...
            Log.Info(spawned ? "Object pool '{0}' create and spawned '{1}'." : "Object pool '{0}' create '{1}'.", TextUtil.GetFullName<T>(Name), obj.Name);
        ...
        public T Spawn(string name)
        {
            foreach (Object<T> obj in m_Objects)
            {
                ...
                if (m_AllowMultiSpawn || !obj.IsInUse)
                {
                    Log.Info("Object pool '{0}' spawn '{1}'.", TextUtil.GetFullName<T>(Name), obj.Peek().Name);
                    return obj.Spawn();
                }
            }
        ...
        public void Unspawn(object target)
        {
            ...
            foreach (Object<T> obj in m_Objects)
            {
                ...
                {
                    Log.Info("Object pool '{0}' unspawn '{1}'.", TextUtil.GetFullName<T>(Name), obj.Peek().Name);
                    obj.Unspawn();
                    Release();
                    return;
                }
            ...
            throw new Exception(TextUtil.Format("Can not find target in object pool '{0}'.", TextUtil.GetFullName<T>(Name)));
        }
        ...
        public void SetLocked(object target, bool locked)
        {
            ...
            foreach (Object<T> obj in m_Objects)
            {
                if (obj.Peek().Target == target)
                {
                    Log.Info("Object pool '{0}' set locked '{1}' to '{2}.", TextUtil.GetFullName<T>(Name), obj.Peek().Name, locked.ToString());
                    obj.Locked = locked;
                    return;
                }
            }
        ...
        public void SetPriority(object target, int priority)
        {
            ...
            foreach (Object<T> obj in m_Objects)
            {
                if (obj.Peek().Target == target)
                {
                    Log.Info("Object pool '{0}' set priority '{1}' to '{2}.", TextUtil.GetFullName<T>(Name), obj.Peek().Name, priority.ToString());
        ...
        public void Release(int toReleaseCount, ReleaseObjectFilterCallback<T> releaseObjectFilterCallback)
        {
            ...
            foreach (ObjectBase toReleaseObject in toReleaseObjects)
            {
                ...
                foreach (Object<T> obj in m_Objects)
                {
                    ...
                    obj.Release(false);
                    Log.Info("Object pool '{0}' release '{1}'.", TextUtil.GetFullName<T>(Name), toReleaseObject.Name);
                    found = true;
                    break;
                }
        ...
        public override void ReleaseAllUnused()
        {
            ...
            while (current != null)
            {
                ...
                Log.Info("Object pool '{0}' release '{1}'.", TextUtil.GetFullName<T>(Name), current.Value.Name);
                current = next;
            }
        }
        ...
        internal override void OnUpdate(float deltaTime, float unscaledDeltaTime)
        {
            ...
            Log.Info("Object pool '{0}' auto release start.", TextUtil.GetFullName<T>(Name));
            Release();
            Log.Info("Object pool '{0}' auto release complete.", TextUtil.GetFullName<T>(Name));
        }
