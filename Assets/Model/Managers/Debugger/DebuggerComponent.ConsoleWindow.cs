...
namespace Myth
{
    public partial class DebuggerComponent
    {
        ...
        private partial class ConsoleWindow : IDebuggerWindow
        {
            ...
            {
                m_SettingComponent = GameEntry.GetBaseComponent<SettingComponent>();
                if (m_SettingComponent == null)
                {
                    Debug.LogWarning("Setting component is invalid.");
                    return;
                }
            ...
            public void OnDraw()
            {
                ...
                GUILayout.BeginHorizontal();
                {
                    ...
                    m_InfoFilter = GUILayout.Toggle(m_InfoFilter, TextUtil.Format("Info ({0})", m_InfoCount.ToString()), GUILayout.Width(90f));
                    m_WarningFilter = GUILayout.Toggle(m_WarningFilter, TextUtil.Format("Warning ({0})", m_WarningCount.ToString()), GUILayout.Width(90f));
                    m_ErrorFilter = GUILayout.Toggle(m_ErrorFilter, TextUtil.Format("Error ({0})", m_ErrorCount.ToString()), GUILayout.Width(90f));
                    m_FatalFilter = GUILayout.Toggle(m_FatalFilter, TextUtil.Format("Fatal ({0})", m_FatalCount.ToString()), GUILayout.Width(90f));
                }
                ...
                GUILayout.BeginVertical("box");
                {
                    m_StackScrollPosition = GUILayout.BeginScrollView(m_StackScrollPosition, GUILayout.Height(100f));
                    {
                        if (m_SelectedNode != null)
                        {
                            ...
                            GUILayout.Label(TextUtil.Format("<color=#{0}{1}{2}{3}><b>{4}</b></color>", color.r.ToString("x2"), color.g.ToString("x2"), color.b.ToString("x2"), color.a.ToString("x2"), m_SelectedNode.LogMessage));
                            if (GUILayout.Button("COPY", GUILayout.Width(60f), GUILayout.Height(30f)))
                            {
                                TextEditor textEditor = new TextEditor();
                                textEditor.text = TextUtil.Format("{0}\n\n{1}", m_SelectedNode.LogMessage, m_SelectedNode.StackTrack);
                                textEditor.OnFocus();
                                textEditor.Copy();
                            }
                            GUILayout.EndHorizontal();
            ...
            private void OnLogMessageReceived(string logMessage, string stackTrace, LogType logType)
            {
                ...
                m_LogNodes.Enqueue(GameEntry.Pool.SpawnClassObject<LogNode>().Fill(logType, logMessage, stackTrace));
                while (m_LogNodes.Count > m_MaxLine)
                {
                    GameEntry.Pool.UnSpawnClassObject(m_LogNodes.Dequeue());
                }
            }
            ...
            private string GetLogString(LogNode logNode)
            {
                ...
                return TextUtil.Format("<color=#{0}{1}{2}{3}>{4}{5}</color>",
